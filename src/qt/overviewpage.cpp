...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeScoreAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeScoreAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeScoreAmount = strAnonymizeScoreAmount.remove(strAnonymizeScoreAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeScoreAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeScoreAmount*COIN) nMaxToAnonymize = nAnonymizeScoreAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeScoreAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeScoreAmount));
        strAnonymizeScoreAmount = strAnonymizeScoreAmount.remove(strAnonymizeScoreAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeScoreAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
