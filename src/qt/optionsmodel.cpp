...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeScoreAmount"))
        settings.setValue("nAnonymizeScoreAmount", 1000);
    nAnonymizeScoreAmount = settings.value("nAnonymizeScoreAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeScoreAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeScoreAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeScoreAmount:
            return QVariant(nAnonymizeScoreAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeScoreAmount:
            nAnonymizeScoreAmount = value.toInt();
            settings.setValue("nAnonymizeScoreAmount", nAnonymizeScoreAmount);
            emit AnonymizeScoreAmountChanged(nAnonymizeScoreAmount);
            break;
        default:
            break;
        }
