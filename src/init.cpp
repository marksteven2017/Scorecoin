...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Score.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Scored.pid)") + "\n";
    ...
    strUsage += "  -scorednsseed          " + _("Always query for peer addresses via DNS lookup (default: 0)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeScoreamount=<n> " + _("Keep N Score anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Score is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Score is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Score version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Score server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Score") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Score to complete") << "\n";
    ...
    nAnonymizeScoreAmount = GetArg("-anonymizeScoreamount", 0);
    if(nAnonymizeScoreAmount > 999999) nAnonymizeScoreAmount = 999999;
    if(nAnonymizeScoreAmount < 2) nAnonymizeScoreAmount = 2;
    ...
    LogPrintf("Anonymize Score Amount %d\n", nAnonymizeScoreAmount);
