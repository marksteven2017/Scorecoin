...
void ProcessMessageDarksend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsf") { //DarkSend Final tx
		...
		if(GetTransaction(i.prevout.hash, tx2, hash)){
			...
			CTxMemPool AcceptableInputs;

	    if(!AcceptableInputs.accept(state, tx,false, pfMissingInputs,false)){
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Score and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002Score per transaction on average.
            */
            if(r <= 20)
            {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeScoreAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fScoreSubmitted = (sessionTotalValue / CENT);
        printf("Submitting Darksend for %f Score CENT - sessionTotalValue %d\n", fScoreSubmitted, sessionTotalValue);
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeScoreAmount = (rand() % nLeftToAnon)+3;
                } else {
