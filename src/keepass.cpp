...
std::string CKeePassIntegrator::constructHTTPPost(const std::string& strMsg, const std::map<std::string,std::string>& mapRequestHeaders)
{
    std::ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: Score-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: localhost\r\n"
...
void CKeePassIntegrator::doHTTPPost(const std::string& sRequest, int& nStatus, std::string& sResponse)
{
    ...
    //if(fDebug) printf("CKeePassIntegrator::doHTTPPost - send POST data: %s\n", strPost.c_str());
    if(fDebug) printf("CKeePassIntegrator::doHTTPPost - send POST data\n");
    ...
    if(fDebug) printf("CKeePassIntegrator::doHTTPPost - request written\n");
    ...
    if(fDebug) printf("CKeePassIntegrator::doHTTPPost - request status line read\n");
    ...
    if(fDebug) printf("CKeePassIntegrator::doHTTPPost - reading response body start\n");
    ...
    if(fDebug) printf("CKeePassIntegrator::doHTTPPost - reading response body end\n");

    // Receive HTTP reply message headers and body
    std::map<std::string, std::string> mapHeaders;
    ReadHTTPMessage(response_stream, mapHeaders, sResponse, nProto, MAX_SIZE);
    if(fDebug) printf("CKeePassIntegrator::doHTTPPost - Processed body\n");
}
...
void CKeePassIntegrator::rpcTestAssociation(bool bTriggerUnlock)
{
    ...
    if(fDebug) printf("CKeePassIntegrator::rpcTestAssociation - send result: status: %d response: %s\n", nStatus, sResponse.c_str());
}
...
std::vector<CKeePassIntegrator::CKeePassEntry> CKeePassIntegrator::rpcGetLogins()
{
    ...
    //if(fDebug) printf("CKeePassIntegrator::rpcGetLogins - send result: status: %d response: %s\n", nStatus, sResponse.c_str());
    if(fDebug) printf("CKeePassIntegrator::rpcGetLogins - send result: status: %d\n", nStatus);
...
void CKeePassIntegrator::rpcSetLogin(const SecureString& strWalletPass, const SecureString& sEntryId)
{
    ...
    if(fDebug) printf("CKeePassIntegrator::rpcSetLogin - send Url: %s\n", sUrl.c_str());

    //request.addStrParameter("SubmitUrl", sSubmitUrl); // Is used to construct the entry title
    request.addStrParameter("Login", SecureString("Score"));
    ...
    if(fDebug) printf("CKeePassIntegrator::rpcSetLogin - send result: status: %d response: %s\n", nStatus, sResponse.c_str());
...
void CKeePassIntegrator::rpcAssociate(std::string& sId, SecureString& sKeyBase64)
{
    ...
    if(fDebug) printf("CKeePassIntegrator::rpcAssociate - send result: status: %d response: %s\n", nStatus, sResponse.c_str());
