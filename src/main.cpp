...
const string strMessageMagic = "Score Signed Message:\n";
...
void ResendWalletTransactions(bool fScore) {
    g_signals.Broadcast(fScore);
}
...
string getDevAddress(int nHeight) {
	int addrId = nHeight % 2;
	if (addrId == 0) {
		return "SVx393Hfw7hixavAwxXNfaLw5A64FiZSon";
	} else {
		return "STLP99YF8FkdSRCzgLBGKwapUdFBRbQaV7";
	}
}
...
{
	int64_t nSubsidy = 0;
  if (pindexBest->nHeight+1 > 2 && pindexBest->nHeight+1 <= 80000) {
		nSubsidy = 42 * COIN;
	}
	else if (pindexBest->nHeight+1 > 80000 && pindexBest->nHeight+1 <= 180000) {
		nSubsidy = 30 * COIN;
	}
	else if (pindexBest->nHeight+1 > 180000 && pindexBest->nHeight+1 <= 320000) {
		nSubsidy = 20 * COIN;
	}
...
bool CBlock::AcceptBlock()
{
    ...
    // Enscore rule that the coinbase starts with serialized block height
