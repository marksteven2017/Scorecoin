...
//
// Bootup the masternode, look for a 25000 SCORE input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Score.conf */
...
{
    CScorecoinAddress address(collateralAddress);
